(function () {
  ...
  function icon(name) {
    const paths = {
      ...
      facebook: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 8h3V4h-3c-3.3 0-5 1.9-5 5v3H6v4h3v5h4v-5h3.2l.8-4H13V9c0-.7.3-1 1-1Z"></path></svg>',
  ...
  function renderSocial() {
    document.querySelectorAll('[data-social]').forEach(el => {
      el.innerHTML = data.social.map(s => `<a class="social-pill" href="${s.href}" target="_blank" rel="noopener">${icon(s.icon)}<span>${s.label}</span></a>`).join('');
    });
  }

  function renderMentors() {
    document.querySelectorAll('[data-mentors]').forEach(el => {
      el.innerHTML = data.mentors.map(m => `
  ...
  function renderSponsors() {
    document.querySelectorAll('[data-sponsors]').forEach(el => {
      el.innerHTML = data.sponsors.map(s => `<article class="sponsor-card"><div class="sponsor-logo">${s.image ? `<img src="${s.image}" alt="${s.name}" loading="lazy" decoding="async">` : `<span>${s.name}</span>`}</div><div><span class="eyebrow">${s.tier}</span><h3>${s.name}</h3></div></article>`).join('');
    });
  }

  function renderGallery() {
    document.querySelectorAll('[data-gallery]').forEach(el => {
      el.innerHTML = data.gallery.map((g, i) => `<figure class="gallery-item ${i === 0 ? 'gallery-feature' : ''}"><img src="${g.src}" alt="${g.alt}" loading="${i === 0 ? 'eager' : 'lazy'}" fetchpriority="${i === 0 ? 'high' : 'auto'}" decoding="async"><figcaption>${g.label}</figcaption></figure>`).join('');
    });
  }
  ...
  function renderTeam() {
    document.querySelectorAll('[data-team-grid]').forEach(el => {
      ...
      el.innerHTML = data.teamMembers.map(m => `<article class="person-card team-card"><div class="person-photo"> <img src="${m.image}" alt="${m.name}" loading="lazy" decoding="async" width="720" height="900"></div><div class="person-body"><span class="eyebrow">${m.role}</span><h3>${m.name}</h3><p>${m.bio}</p><div class="tag-row">${(m.skills || []).map(x => `<span>${x}</span>`).join('')}</div></div></article>`).join('');
    });
  }

  function renderSeasons() {
    document.querySelectorAll('[data-seasons]').forEach(el => {
      el.innerHTML = data.seasons.map((s, i) => `<article class="timeline-item"><div class="timeline-year">${s.year}</div><div><span class="eyebrow">${i === 0 ? 'Recent season' : 'Program history'}</span><h3>${s.title}</h3><p>${s.description}</p><div class="tag-row">${s.highlights.map(x => `<span>${x}</span>`).join('')}</div></div></article>`).join('');
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    renderShell(); renderSocial(); renderMentors(); renderSponsors(); renderGallery(); renderTeam(); renderSeasons(); setupTheme(); setupMenu(); markActiveNav();
  });
})();
